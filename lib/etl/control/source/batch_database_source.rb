...
module ETL #:nodoc:
  ...
  module Control #:nodoc:
    ...
    class BatchDatabaseSource < Source
      ...
      def initialize(control, configuration, definition)
        ...
        @batch_start = configuration[:batch_start] || 0
        @batch_size = configuration[:batch_size] || 1000
        @batch_column = configuration[:batch_column] || "id"
      end
      ...
      def columns
        ...
        @columns ||= query_rows.any? ? (query_rows.first.keys - [batch_id_column]) : ['']
      end
      ...
      def find_each
        ...
        while records.any?
          ...
          @last_value = records.last[batch_id_column]
          @query_rows = nil
          records = query_rows
        end
      end
      ...
      # this is the one that needs some edits
      ...
      def query
        q = "SELECT #{select}, #{configuration[:table]}.#{@batch_column} as `#{batch_id_column}` FROM #{configuration[:table]}"
        q << " #{join}" if join
        
        conditions = []
        
        # add the batch condition if there is a last_value specified
        conditions << "#{configuration[:table]}.#{@batch_column} > #{@last_value.to_i}" if @last_value
        ...
        q << " GROUP BY #{group}, #{configuration[:table]}.#{@batch_column}" if group
        q << " ORDER BY #{configuration[:table]}.#{@batch_column} ASC"

        # add in the batch size
        connection.add_limit_offset!(q, {:limit=>@batch_size})
